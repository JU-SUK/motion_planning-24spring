...
def readmap():
    ...
    with open('P1_map.txt', 'r') as f:
        for line in f:
            ...
            for c in l:
                if c == 'S':
                    mapinfo.update({'start': P1_astar.Point(l.index(c), h - 1)})
                    l_t[-1][l.index(c)] = 0

                if c == 'E':
                    mapinfo.update({'end': P1_astar.Point(l.index(c), h - 1)})
                    l_t[-1][l.index(c)] = 0
    l_t_t = list()
    for i in range(w):
        t = list()
        for j in range(h):
            t.append(l_t[j][i])
        l_t_t.append(t)
    map2d = P1_astar.Array2D(w, h, l_t_t)
    mapinfo.update({'map2d': map2d})
    return mapinfo

if __name__ == "__main__":
    mapinfo = readmap()
    aStar = P1_astar.AStar(mapinfo['map2d'], mapinfo['start'], mapinfo['end'])
