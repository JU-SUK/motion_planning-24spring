...
class Animation:
  def __init__(self, map, schedule):
    ...
    self.colors = ['orange', 'blue', 'green', 'purple', 'brown', 'pink', 'gray', 'olive', 'cyan', 'magenta']
    self.agent_paths = {name: {'x': [], 'y': [], 'line': None} for name in self.combined_schedule.keys()}
    ...
    # 각 에이전트의 경로를 그리기 위한 Line2D 객체 초기화
    for name, path in self.agent_paths.items():
      path['line'] = Line2D([], [], color=self.agents[name].original_face_color, linewidth=2, alpha=0.5)
      self.ax.add_line(path['line'])
  ...
  def animate_func(self, i):
    ...
    for agent_name, agent_path in self.agent_paths.items():
      pos = self.getState(i / 10, self.combined_schedule[agent_name])
      agent_path['x'].append(pos[0])
      agent_path['y'].append(pos[1])
      agent_path['line'].set_data(agent_path['x'], agent_path['y'])
      print(f"{agent_name} path data:", agent_path['line'].get_data())
    ...
    return self.patches + self.artists + [agent_path['line'] for _, agent_path in self.agent_paths.items()]
...
if __name__ == "__main__":
  ...
  parser.add_argument("map", type=str, default="input.yaml",help="input file containing map")
  parser.add_argument("schedule", type=str, default="output.yaml", help="schedule for agents")
